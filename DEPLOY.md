...
2. Settings → Domains에 `baby.lifebanjang.com` 추가, 안내되는 CNAME을 도메인 DNS에 등록
...
- 각 배치 지점 slot 번호(예: `parental-leave-below-tool`)를 애드센스 콘솔에서 발급받아 `AdSlot`에 전달

## 허브 등록 (신규 노트 공통)

- `lifebanjang-hub/lib/notes.ts`에 육아노트 카드 항목 추가
- 각 노트의 `components/FamilyLinks.tsx` SITES 배열에 baby 항목 추가(전 사이트 크로스링크)
